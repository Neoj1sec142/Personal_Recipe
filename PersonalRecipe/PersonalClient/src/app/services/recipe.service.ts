...
export class RecipeService {
  ...
  get(id: number){ this.http.get<Recipe>(this.apiUrl + `${id}/`) };
  create(c: Recipe){ this.http.post<Recipe>(this.apiUrl, c) };
  quickCreate(c: Recipe){ this.http.post<Recipe>(this.apiUrl + `quick/`, c) };
  update(c: Recipe){ this.http.put(this.apiUrl + `${c.id}/`, c) };
  delete(id: number){ this.http.get<Recipe>(this.apiUrl + `${id}/`) };
  getAll(params: ListParams){
    ...
    return this.http.post<Recipe[]>(this.apiUrl, {...headers});
  }
}
