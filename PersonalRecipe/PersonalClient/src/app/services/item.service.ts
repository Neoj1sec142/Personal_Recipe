...
export class ItemService {
  ...
  get(id: number){ this.http.get<Item>(this.apiUrl + `${id}/`) };
  create(c: Item){ this.http.post<Item>(this.apiUrl + `${c.id}/`, c) };
  update(c: Item){ this.http.put(this.apiUrl + `${c.id}/`, c) };
  delete(id: number){ this.http.get<Item>(this.apiUrl + `${id}/`) };
  getAll(params: ListParams){
    ...
    return this.http.post<Item[]>(this.apiUrl, {...headers});
  }
}
